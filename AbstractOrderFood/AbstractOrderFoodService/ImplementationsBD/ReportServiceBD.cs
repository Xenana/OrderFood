   ...
   public class ReportServiceBD : IReportService
    {
        ...
        public void SaveCourseSetPrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                var courseSets = context.CourseSet.ToList();
                //создаем таблицу
                var paragraphTable = document.Paragraphs.Add(Type.Missing);
                var rangeTable = paragraphTable.Range;
                var table = document.Tables.Add(rangeTable, courseSets.Count, 2, ref missing, ref missing);
                ...
                for (int i = 0; i < courseSets.Count; ++i)
                {
                    table.Cell(i + 1, 1).Range.Text = courseSets[i].CourseSetsName;
                    table.Cell(i + 1, 2).Range.Text = courseSets[i].Cost.ToString();
                }
        ...
        public List<KitchensLoadViewModel> GetKitchensLoad()
        {
            return context.Kitchens
                            .ToList()
                            .GroupJoin(
                                    context.KitchenCourse
                                                .Include(r => r.Courses)
                                                .ToList(),
                                    kitchen => kitchen,
                                    kitchenCourse => kitchenCourse.Kitchen,
                                    (kitchen, kitchenCourList) =>
            new KitchensLoadViewModel
            {
                KitchenName = kitchen.KitchenName,
                TotalCount = kitchenCourList.Sum(r => r.Count),
                Courses = kitchenCourList.Select(r => new Tuple<string, int>(r.Courses.CoursesName, r.Count))
            })
                            .ToList();
        }
        ...
        public void SaveKitchensLoad(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                excelcells.Value2 = "Загруженность кухонь";
        ...
        public List<CustomerBasketsModel> GetCustomerBaskets(ReportBindingModel model)
        {
            return context.Baskets
                            .Include(rec => rec.Customers)
                            .Include(rec => rec.CourseSets)
                            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo)
                            .Select(rec => new CustomerBasketsModel
                            {
                                CustomerName = rec.Customers.CustomersFIO,
                                DateCreate = SqlFunctions.DateName("dd", rec.DateCreate) + " " +
                                            SqlFunctions.DateName("mm", rec.DateCreate) + " " +
                                            SqlFunctions.DateName("yyyy", rec.DateCreate),
                                CourseSetName = rec.CourseSets.CourseSetsName,
                                Count = rec.Count,
                                Sum = rec.Sum,
        ...
        public void SaveCustomerBaskets(ReportBindingModel model)
        {
            ...
            //вставляем заголовок
            var phraseTitle = new Phrase("Заказы заказчиков",
            ...
            table.AddCell(new PdfPCell(new Phrase("Блюда", fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_CENTER
            });
            ...
            //заполняем таблицу
            var list = GetCustomerBaskets(model);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].CourseSetName, fontForCells));
                table.AddCell(cell);
